...
class GoogleOAuthService:
    ...
    def __init__(self):
        ...
        self.redirect_uri = os.getenv(
            "GOOGLE_REDIRECT_URI", "http://localhost:8000/auth/google/auth/callback"
        )
    ...
    async def get_gmail_messages(
        ...
        try:
            ...
            logger.error(f"Error getting Gmail messages: {e}", exc_info=True)
    ...
    def _extract_email_body(self, payload: Dict[str, Any]) -> str:
        ...
        def decode_base64url(data: str) -> str:
            ...
            try:
                ...
                logger.error(f"Error decoding base64url: {e}", exc_info=True)
                return ""
    ...
    async def get_gmail_message_content(
        ...
        try:
            ...
            logger.error(f"Error getting Gmail message content: {e}", exc_info=True)
            return {"error": str(e)}
    ...
    async def search_drive_files(
        ...
        try:
            ...
            if search_term:
                query_parts.append(
                    f"(fullText contains '{search_term}' OR name contains '{search_term}')"
                )
            ...
            if file_type:
                if file_type.lower() in ["photo", "photos", "image", "images"]:
                    ...
                    query_parts.append(
                        "(mimeType contains 'document' OR mimeType contains 'pdf')"
                    )
            ...
            if year:
                ...
                query_parts.append(
                    f"modifiedTime >= '{start_date}' AND modifiedTime < '{end_date}'"
                )
    ...
    async def search_drive_folders(
        ...
        try:
            ...
            logger.error(f"Error searching Drive folders: {e}", exc_info=True)
    ...
    async def create_folder_structure(
        ...
        try:
            ...
            path_parts = [root_folder] + [
                part.strip() for part in folder_path.split("/") if part.strip()
            ]

            current_parent = None
            for folder_name in path_parts:
                folder_id = await self._get_or_create_folder(
                    service, folder_name, current_parent
                )
            ...
            logger.error(f"Error creating folder structure: {e}", exc_info=True)
            raise
    ...
    async def _get_or_create_folder(
        ...
        try:
            ...
            logger.error(
                f"Error getting/creating folder '{folder_name}': {e}", exc_info=True
            )
            raise
    ...
    async def upload_file_to_drive(
        ...
        try:
            ...
            if folder_path:
                parent_folder_id = await self.create_folder_structure(
    ...
    async def get_shared_drives(
        ...
        try:
            ...
            logger.error(f"Error getting shared drives: {e}", exc_info=True)
