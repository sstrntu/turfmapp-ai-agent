...
from ...core.rate_limiting import (
    apply_oauth_rate_limit,
    apply_google_api_rate_limit,
...
def get_frontend_url() -> str:
    """Get frontend URL from environment or default to localhost:3005"""
    frontend_url = os.getenv("FRONTEND_URL", "http://localhost:3005")
    print(f"🌐 [DEBUG] Using frontend URL: {frontend_url}")
    return frontend_url
...
async def get_google_auth_url(
    ...
    print(
        f"🔐 [DEBUG] current_user: {current_user.get('email', 'N/A')} (ID: {current_user.get('id', 'N/A')})"
    )
    ...
    try:
        ...
        print(
            f"🔧 [DEBUG] Calling google_oauth_service.get_authorization_url with state: {state_data}"
        )
...
async def handle_google_callback_get(
    ...
    if not code:
        # Redirect to frontend with error
        return RedirectResponse(
            url=f"{frontend_url}/google-callback.html?error=no_code", status_code=302
        )
...
async def handle_google_callback(
    ...
    print(
        f"📋 [DEBUG] request_data: code={request_data.code[:20]}..., state={request_data.state}"
    )
    print(
        f"🔐 [DEBUG] current_user: {current_user.get('email', 'N/A')} (ID: {current_user.get('id', 'N/A')})"
    )
    ...
    try:
        ...
        try:
            ...
            print(
                f"🏷️ [DEBUG] Parsed state - user_id: {state_user_id}, action: {action}"
            )
        except Exception as state_error:
            print(
                f"⚠️ [DEBUG] State parsing failed: {state_error}, using backward compatibility"
            )
        ...
        print(
            f"🔍 [DEBUG] Verifying state - state_user_id: {state_user_id}, current_user_id: {current_user_id}"
        )
        ...
        token_data = await google_oauth_service.exchange_code_for_tokens(
            request_data.code, request_data.state
        )
...
async def get_google_auth_status(
    ...
    try:
        ...
        print(
            f"👑 [DEBUG] Primary account: {primary_account.email if primary_account else 'None'}"
        )
        ...
        return GoogleAuthResponse(
            success=True, message="Auth status retrieved", data=response_data
        )
...
async def refresh_google_tokens(
    request: Request, current_user: Dict[str, Any] = Depends(get_current_user_supabase)
):
    ...
    try:
        ...
        new_tokens = google_oauth_service.refresh_access_token(
            primary_account.tokens.refresh_token
        )
        if not new_tokens:
        ...
        updated_tokens = GoogleTokens(
            ...
            or primary_account.tokens.refresh_token,
            expires_at=new_tokens["expires_in"],
        )

        success = await google_accounts_db.update_tokens(
            user_id_str, primary_account.email, updated_tokens
        )

        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to update tokens in database"
            )
...
async def refresh_account_tokens(
    ...
    try:
        ...
        account = await google_accounts_db.get_account_by_email(
            user_id_str, account_email
        )
        ...
        success = await google_accounts_db.update_tokens(
            user_id_str, account_email, updated_tokens
        )
        ...
        return GoogleAuthResponse(
            ...
            data={"account": account_email, "expires_at": updated_tokens.expires_at},
        )
